package se.johannesdahlgren.adventofcode2021;

import static org.assertj.core.api.Assertions.assertThat;

import org.junit.jupiter.api.Test;

class Day16Test {

  @Test
  void part1Example1() {
    Day16 day16 = new Day16("D2FE28");
    int versionNumberSum = day16.getVersionNumberSum();
    assertThat(versionNumberSum).isEqualTo(6);
  }

  @Test
  void part1Example2() {
    Day16 day16 = new Day16("38006F45291200");
    int versionNumberSum = day16.getVersionNumberSum();
    assertThat(versionNumberSum).isEqualTo(9);
  }

  @Test
  void part1Example3() {
    Day16 day16 = new Day16("EE00D40C823060");
    int versionNumberSum = day16.getVersionNumberSum();
    assertThat(versionNumberSum).isEqualTo(14);
  }

  @Test
  void part1Example4() {
    Day16 day16 = new Day16("8A004A801A8002F478");
    int versionNumberSum = day16.getVersionNumberSum();
    assertThat(versionNumberSum).isEqualTo(16);
  }

  @Test
  void part1Example5() {
    Day16 day16 = new Day16("620080001611562C8802118E34");
    int versionNumberSum = day16.getVersionNumberSum();
    assertThat(versionNumberSum).isEqualTo(12);
  }

  @Test
  void part1Example6() {
    Day16 day16 = new Day16("C0015000016115A2E0802F182340");
    int versionNumberSum = day16.getVersionNumberSum();
    assertThat(versionNumberSum).isEqualTo(23);
  }

  @Test
  void part1Example7() {
    Day16 day16 = new Day16("A0016C880162017C3686B18A3D4780");
    int versionNumberSum = day16.getVersionNumberSum();
    assertThat(versionNumberSum).isEqualTo(31);
  }

  @Test
  void part1() {
    Day16 day16 = new Day16("");
    int versionNumberSum = day16.getVersionNumberSum();
    assertThat(versionNumberSum).isEqualTo(993);
  }
}